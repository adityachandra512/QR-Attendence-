...
@app.route('/data_sports', methods=['GET', 'POST'])
def data_sports():
    f = "sports.csv"
    data = []
    with open(f) as file:
...
@app.route('/video_feed_sports')
def video_feed_sports():
    return Response(gen_frames_sports(), mimetype='multipart/x-mixed-replace; boundary=frame')
